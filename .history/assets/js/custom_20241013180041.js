...
// Función para actualizar el estilo del menú basado en el tamaño de la ventana
function updateMenuStyle() {
    const menu = document.querySelector('header.style-2 .main-menu');
    if (window.innerWidth >= 992) {
      // En pantallas grandes, desactiva cualquier restricción
      menu.style.maxHeight = 'none';
      menu.style.overflowY = 'visible'; // No necesita scroll
    } else {
      // En pantallas pequeñas, aplica las restricciones
      menu.style.maxHeight = '300px';
      menu.style.overflowY = 'auto'; // Activa el scroll
    }
  }
  
  // Escuchar el evento 'resize' para detectar cuando se cambia el tamaño de la pantalla
  window.addEventListener('resize', updateMenuStyle);
  
  // Ejecutar la función en la carga de la página para que los estilos se apliquen correctamente
  window.addEventListener('DOMContentLoaded', updateMenuStyle);
